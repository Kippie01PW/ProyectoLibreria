		    ...
		    request.onreadystatechange = function(){
		    	...
		    	if(request.status == 200){
		    		...
		    		if(objData.status){
		    		 ...
		    		 //window.location.href = "http://localhost/tienda_viertual/Tienda/confirmarpedido";
		    		}else{
		    			swal("", objData.msg , "error");
		    		}
		    	}
...
if(document.querySelector("#btnComprar")){
	...
	btnPago.addEventListener('click', function() { 
		...
		if(dir.trim() === "" || ciudad.trim() === "" || inttipopago.trim() === ""){
			swal("", "Complete datos de envío", "error");
			return;
		}
		...
		let ajaxUrl = base_url + '/Tienda/procesarVenta';
		//let ajaxUrl = base_url + '/Tienda/procesarpago';
		let formData = new FormData();
		formData.append('direccion', dir);    
		formData.append('ciudad', ciudad);
		formData.append('inttipopago', inttipopago);
